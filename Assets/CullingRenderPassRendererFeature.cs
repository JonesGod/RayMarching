...
public class CullingRenderPass : ScriptableRenderPass
{
    ...
    public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
    {
        ...
        using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData, profilingSampler))
        {
            ...
            //using custom RenderTarget
            var customTextureDesc = new TextureDesc(Vector2.one * 1024)
            {
                colorFormat = GraphicsFormat.R8G8B8A8_UNorm,
                enableRandomWrite = true,
                clearBuffer = true,
                clearColor = Color.clear,
                name = "CustomRenderTarget"
            };
            var customTextureHandle = renderGraph.CreateTexture(customTextureDesc);
            builder.SetRenderAttachment(customTextureHandle, 0);
            builder.UseTexture(customTextureHandle, AccessFlags.ReadWrite);
